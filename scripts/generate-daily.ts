...
for(const row of existing||[]){const ids=(row.daily_challenge_questions as any[]).slice().sort((a,b)=>a.position-b.position).map(x=>x.question_id as string);if(ids.length!==10||new Set(ids).size!==10)throw new Error(`Daily generation failed: existing published challenge ${row.challenge_date} is malformed.`);scheduled.set(row.challenge_date,ids)}

const used:string[][]=[];
for(const row of existing||[]){if(row.challenge_date<startKey){const ids=scheduled.get(row.challenge_date)!;used.push(ids)}}
...
for(let day=0;day<90;day++){
  ...
  if(already){if(already.some(id=>blocked.has(id)))throw new Error(`Daily generation failed: existing published challenge ${keyDate} violates the 21-day exclusion.`);if(already.some(id=>!eligibleById.has(id)))throw new Error(`Daily generation failed: existing published challenge ${keyDate} contains an ineligible question.`);used.push(already);continue}
